...
| --- | --- |
| `auto` | 不强制尺寸，由上游决定。 |
| `1080p` / `2k` / `4k` | 仅作为画质档位提示加入 prompt，不下发具体像素。上游（codex/responses 与 OpenAI Images API）只接受 `1024x1024 / 1536x1024 / 1024x1536 / auto`，超出值会被打回 5xx。 |
| `1:1`、`3:2`、`2:3`、`16:9`、`21:9`、`9:16`、`4:3`、`3:4` | 作为构图比例提示。 |
| `1024x1024`、`1536x1024`、`1024x1536` | 显式宽高，按上游白名单原样下发；其它像素会被收敛到最接近的白名单档位。 |

异步任务还支持 `image_resolution` 元数据字段，取值为 `1080p`、`2k`、`4k`。该字段仅用于记录分辨率档位和图库元数据，不替代 `size`，也不会进入上游请求体。
